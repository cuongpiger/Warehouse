...
where year(dt.NGAYKT) = 2010
group by cd.TENCD

--q2
select dt.TENDT, count(distinct tgdt.MAGV)
from DETAI dt join THAMGIADT tgdt on dt.MADT = tgdt.MADT
group by dt.TENDT

--q3
select bm.TENBM, count(gv.MAGV)
...
where dt.CAPQL = N'Tru?ng' and not exists (select gv1.MAGV
										   from GIAOVIEN gv1 join THAMGIADT tgdt1 on gv1.MAGV = tgdt1.MAGV join DETAI dt1 on tgdt1.MADT = dt1.MADT
										   where dt1.CAPQL = N'ÐHQG' and gv1.MAGV = gv.MAGV)

--q5
select bm.TENBM
from GIAOVIEN gv join BOMON bm on gv.MABM = bm.MABM join DETAI dt on gv.MAGV = dt.GVCDDT
group by bm.TENBM
having count(distinct gv.MAGV) >= 3
...
having count(distinct gv.MAGV) >= all(select count(distinct gv1.MAGV)
								      from GIAOVIEN gv1 join BOMON bm1 on gv1.MABM = bm1.MABM join DETAI dt1 on gv1.MAGV = dt1.GVCDDT
								      group by bm1.MABM)

--q7
select gv.MAGV, gv.HOTEN, count(distinct dt.MADT)
from GIAOVIEN gv join BOMON bm on bm.TRUONGBM = gv.MAGV left join DETAI dt on dt.GVCDDT = gv.MAGV
group by gv.MAGV, gv.HOTEN

--q8
select gv1.HOTEN
from GIAOVIEN gv1 join BOMON bm1 on bm1.TRUONGBM = gv1.MAGV
except
select gv.HOTEN
from GIAOVIEN gv join BOMON bm on bm.TRUONGBM = gv.MAGV left join DETAI dt on dt.GVCDDT = gv.MAGV
group by gv.HOTEN
...
--q9
select distinct *
from CONGVIEC cv join THAMGIADT tgdt on cv.MADT = tgdt.MADT join GIAOVIEN gv on gv.MAGV = tgdt.MAGV

select gv1.HOTEN
from GIAOVIEN gv1
where exists (select *
			  from CONGVIEC cv join THAMGIADT tgdt on (cv.MADT = tgdt.MADT and tgdt.STT = cv.SOTT) join GIAOVIEN gv on (gv.MAGV = tgdt.MAGV)
			  where cv.TENCV = N'Thiết kệ hệ thống' and gv1.MAGV = gv.MAGV)
