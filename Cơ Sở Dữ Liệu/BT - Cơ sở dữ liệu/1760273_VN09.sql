...
where not exists (
				 ...
				 (select tgdt1.MADT
				  ...
				  where tgdt1.MAGV = tgdt.MAGV)
				 )

--q59
select distinct tgdt.MADT, dt.TENDT
from THAMGIADT tgdt join GIAOVIEN gv on tgdt.MAGV = gv.MAGV and gv.MABM = 'HTTT' join DETAI dt on tgdt.MADT = dt.MADT
where not exists ((select tgdt1.MADT
				   from THAMGIADT tgdt1 join GIAOVIEN gv1 on tgdt1.MAGV = gv1.MAGV and gv1.MABM = 'HTTT')
				  except
				  (select tgdt2.MADT
				   from THAMGIADT tgdt2 join GIAOVIEN gv2 on tgdt2.MAGV = gv2.MAGV and gv2.MABM = 'HTTT'
				   where tgdt.MAGV <> tgdt2.MAGV)
				  )
...
--q61
select distinct gv.HOTEN, tgdt.MADT, tgdt.MAGV
from CHUDE cd join DETAI dt on cd.MACD = dt.MACD join THAMGIADT tgdt on dt.MADT = tgdt.MADT join GIAOVIEN gv on gv.MAGV = tgdt.MAGV
where cd.MACD = 'QLGD' and not exists ((select dt1.MADT
										from CHUDE cd1 join DETAI dt1 on cd1.MACD = dt1.MACD and cd1.MACD = 'QLGD')
									   except
									   (select tgdt2.MADT
									    from THAMGIADT tgdt2
										where tgdt2.MADT = tgdt.MADT and tgdt2.MAGV = tgdt.MAGV))
...
having count(distinct tgdt.MADT) = (select count(distinct tgdt2.MADT)
									from GIAOVIEN gv2 join THAMGIADT tgdt2 on gv2.MAGV = tgdt2.MAGV and gv2.HOTEN = N'Trần Trà Hương')
...
where gv.MABM = 'HHC'
group by dt.TENDT
having count(distinct tgdt.MAGV) = (select count(distinct gv1.MAGV)
									from GIAOVIEN gv1
									where gv1.MABM = 'HHC') 

--q64
select gv.MAGV, gv.HOTEN
from GIAOVIEN gv join THAMGIADT tgdt on gv.MAGV = tgdt.MAGV join CONGVIEC cv on cv.MADT = tgdt.MADT and cv.SOTT = tgdt.STT
where cv.MADT = '006'
group by gv.MAGV, gv.HOTEN
having count(distinct cv.SOTT) = (select count(distinct cv1.TENCV)
								  from CONGVIEC cv1
								  where cv1.MADT = '006')

--q65
select gv.MAGV, gv.HOTEN
from GIAOVIEN gv join THAMGIADT tgdt on gv.MAGV = tgdt.MAGV join DETAI dt on dt.MADT = tgdt.MADT join CHUDE cd on cd.MACD = dt.MACD
where cd.MACD = 'UDCN'
group by gv.MAGV, gv.HOTEN
having count(distinct dt.MADT) = (select count(distinct dt1.MADT)
								  from DETAI dt1
								  where dt1.MACD = 'UDCN')
...
select gv.MAGV, gv.HOTEN
from GIAOVIEN gv join THAMGIADT tgdt on gv.MAGV = tgdt.MAGV join DETAI dt on tgdt.MADT = dt.MADT
where dt.GVCDDT = (select distinct gv1.MAGV
				   from GIAOVIEN gv1 join DETAI dt1 on gv1.MAGV = dt1.GVCDDT
				   where gv1.HOTEN = N'Trần Trà Hương')
group by gv.MAGV, gv.HOTEN
having count(distinct tgdt.MADT) = (select count(distinct dt2.MADT)
									from GIAOVIEN gv2 join DETAI dt2 on gv2.MAGV = dt2.GVCDDT
									where gv2.HOTEN = N'Trần Trà Hương')

--q67
select dt.TENDT
from DETAI dt join THAMGIADT tgdt on dt.MADT = tgdt.MADT join GIAOVIEN gv on gv.MAGV = tgdt.MAGV join BOMON bm on gv.MABM = bm.MABM join KHOA k on k.MAKHOA = bm.MAKHOA
where k.TENKHOA = N'Công nghệ thông tin'
group by dt.TENDT
having count(distinct gv.MAGV) = (select count(distinct gv1.MAGV)
							      from GIAOVIEN gv1 join BOMON bm1 on gv1.MABM = bm1.MABM join KHOA k1 on k1.MAKHOA = bm1.MAKHOA
								  where k1.TENKHOA = N'Công nghệ thông tin')

--q68
select gv.MAGV, gv.HOTEN
from GIAOVIEN gv join THAMGIADT tgdt on gv.MAGV = tgdt.MAGV join DETAI dt on tgdt.MADT = dt.MADT join CONGVIEC cv on cv.SOTT = tgdt.STT and cv.MADT = tgdt.MADT
where dt.TENDT = N'Nghiên cứu tế bào gốc'
group by gv.MAGV, gv.HOTEN
having count(distinct tgdt.STT) = (select count(distinct cv1.SOTT)
...
--q70
select dt.TENDT
from GIAOVIEN gv join BOMON bm on gv.MABM = bm.MABM join KHOA k on k.MAKHOA = bm.MAKHOA join THAMGIADT tgdt on gv.MAGV = tgdt.MAGV join DETAI dt on tgdt.MADT = dt.MADT
...
from GIAOVIEN gv join THAMGIADT tgdt on gv.MAGV = tgdt.MAGV join DETAI dt on dt.MADT = tgdt.MADT join CONGVIEC cv on cv.SOTT = tgdt.STT and cv.MADT = tgdt.MADT
where dt.TENDT = N'Ứng dụng hóa học xanh'
...
from GIAOVIEN gv join GIAOVIEN gv1 on gv.GVQLCM = gv1.MAGV join THAMGIADT tgdt on tgdt.MAGV = gv.MAGV join DETAI dt on dt.MADT = tgdt.MADT join CHUDE cd on cd.MACD = dt.MACD join BOMON bm on bm.MABM = gv.MABM 
where cd.TENCD = N'Nghiên cứu và phát triển'
group by gv.MAGV, gv.HOTEN, bm.TENBM, gv1.HOTEN
having count(distinct tgdt.MADT) = (select count(distinct dt1.MADT)
									from CHUDE cd1 join DETAI dt1 on cd1.MACD = dt1.MACD
									where cd1.TENCD = N'Nghiên cứu và phát triển')

--q73
select gv.MAGV, gv.HOTEN, k.TENKHOA, gv1.HOTEN, tgdt.MADT
from GIAOVIEN gv join BOMON bm on bm.MABM = gv.MABM join KHOA k on k.MAKHOA = bm.MAKHOA join GIAOVIEN gv1 on gv1.MAGV = k.TRUONGKHOA join THAMGIADT tgdt on tgdt.MAGV = gv.MAGV
group by gv.MAGV, gv.HOTEN, k.TENKHOA, gv1.HOTEN, tgdt.MADT
having not exists (
				   ...
				   (select distinct tgdt2.MADT
					from THAMGIADT tgdt2 join GIAOVIEN gv3 on gv3.MAGV = tgdt2.MAGV
					where tgdt2.MADT = tgdt.MADT and gv3.HOTEN = N'Nguyễn Hoài An')
				  )

--q74
select gv5.MAGV, gv5.HOTEN
from GIAOVIEN gv5 join BOMON bm5 on gv5.MABM = bm5.MABM join KHOA k5 on k5.MAKHOA = bm5.MAKHOA join THAMGIADT tgdt5 on tgdt5.MAGV = gv5.MAGV
where k5.TENKHOA = N'Công nghệ thông tin'
group by gv5.MAGV, gv5.HOTEN
having not exists (
				  (select distinct tgdt.MADT
				   from (select bm.MABM as y1, bm.TRUONGBM as y2, COUNT(distinct gv.MAGV) as y3
						 from GIAOVIEN gv join BOMON bm on bm.MABM = gv.MABM join KHOA k on k.MAKHOA = bm.MAKHOA
						 where k.TENKHOA = N'Công nghệ thông tin'
						 group by bm.MABM, bm.TRUONGBM) as y join THAMGIADT tgdt on tgdt.MAGV = y.y2
				   where y.y3 >= all (select y1.y2
									  from (select bm1.MABM as y1, COUNT(distinct gv1.MAGV) as y2
					 						from GIAOVIEN gv1 join BOMON bm1 on bm1.MABM = gv1.MABM join KHOA k1 on k1.MAKHOA = bm1.MAKHOA
											where k1.TENKHOA = N'Công nghê thông tin'
											group by bm1.MABM) as y1))
				  except
				  (select distinct tgdt4.MADT
				   from GIAOVIEN gv4 join THAMGIADT tgdt4 on gv4.MAGV = tgdt4.MAGV
				   where gv4.MAGV = gv5.MAGV)
				  )
...
from GIAOVIEN gv left join DETAI dt on gv.MAGV = dt.GVCDDT

--cho danh sách giáo viên tham gia tất cả công việc của đề tài do trường bộ môn HTTT đảm nhiệm
select gv.MAGV, gv.HOTEN
from GIAOVIEN gv join THAMGIADT tgdt on gv.MAGV = tgdt.MAGV
where tgdt.MADT in (select dt1.MADT
					from GIAOVIEN gv1 join DETAI dt1 on gv1.MAGV = dt1.GVCDDT join BOMON bm1 on bm1.TRUONGBM = gv1.MAGV
					where bm1.MABM = 'HTTT')
group by gv.MAGV, gv.HOTEN
having count(tgdt.STT) = (select count(cv2.SOTT)
						  from GIAOVIEN gv2 join DETAI dt2 on gv2.MAGV = dt2.GVCDDT join CONGVIEC cv2 on cv2.MADT = dt2.MADT join BOMON bm2 on bm2.TRUONGBM = gv2.MAGV
						  where bm2.MABM = 'HTTT')
