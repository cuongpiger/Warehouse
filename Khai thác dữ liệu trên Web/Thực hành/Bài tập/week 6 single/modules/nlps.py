...
class NLPs(NLP):
    ...
    def calculateSimilarity(self):
        doc1 = int(input('Enter the index for the main document: '))
        doc2 = int(input('Enter the index for the comparing document: '))

        data = pd.read_csv('results/bow.csv', encoding='utf-8')

        data1 = data.iloc[doc1].to_list()
        data2 = data.iloc[doc2].to_list()
        
        result = 1 - spatial.distance.cosine(data1, data2)

        print('The cosine similarity is: {0}'.format(result))
