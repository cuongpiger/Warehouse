...
router.get("/sign_up/:status", (req, res) => {
  const status = +req.params.status;

  if (status === 0) {
    res.render("../views/vwUsers/signup");
  } else if (status === 1) {
    regUser = undefined;
...
router.post("/sign_up", async (req, res) => {
  regUser = req.body;
  const check = await usersModel.loadAUser(regUser.email);

  // user is not in the database's users field
  if (check.length === 0 && regUser.password === regUser.retypepassword) {
    delete regUser.retypepassword;

    regUser.avatar = '';
    regUser.password = md5(regUser.password);

    await usersModel.insert(regUser);

    res.redirect('/users/sign_up/1');
  } else {
    res.redirect('/users/sign_up/2');
  }
});
