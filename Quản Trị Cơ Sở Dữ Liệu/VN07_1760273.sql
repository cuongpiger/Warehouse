...
/*
5. Vieቷt hà m truyeቹn vào ngày cho bieቷt danh sá ch đaቹu sá ch (ISBN, TenSach) và soቷ
sá ch đã mượ n trong ngà y đó (gọi Func_L1 câu 10 và Func_L2 câu 4)
*/

if OBJECT_ID('Q5') is not null
	drop function Q5
go
create function Q5 (@borrowDate date)
	returns table
as
	return (select ds.isbn, ds.tensach, count(distinct pm.mapm) as amount
			from DauSach ds, PhieuMuon pm, CT_PhieuMuon ctpm
			where ds.isbn = ctpm.isbn and pm.mapm = ctpm.mapm and pm.ngaymuon = @borrowDate
...
/*
6. Vieቷt hà m truyền và o MaDG, cho bieቷt danh sá ch sách đọ c giả đã mượ n
*/
if OBJECT_ID('Q6') is not null
	drop function Q6
go
create function Q6 (@readerID nchar(10))
	returns table
as
	return (select ds.tensach, ds.isbn, cs.masach
			from DocGia dg, PhieuMuon pm, CT_PhieuMuon ctpm, CuonSach cs, DauSach ds
			where dg.madg = @readerID and
				  pm.mapm = ctpm.mapm and
				  ctpm.isbn = cs.isbn and
				  cs.isbn = ds.isbn)
go
...
/*
7. Vieቷt hà m cho bieቷt danh sá ch đọc giả (MaDG, HoTen) và cá c cuoቷn sá ch ( ISBN,
MaSach) mà đọc giả cò n nơ
*/

if OBJECT_ID('Q7') is not null
	drop function Q7
go
create function Q7 ()
	returns table
as
	return (select dg.madg, dg.hoten, cs.isbn, cs.masach
			from DocGia dg, PhieuMuon pm, CT_PhieuMuon ctpm, CuonSach cs
			where dg.madg = pm.madg and
				  pm.mapm = ctpm.mapm and
				  ctpm.isbn = cs.isbn and ctpm.masach = cs.masach and
				  cs.tinhtrang <> N'có thể mượn')
go
...
/*
8. Viết hà m cho bieቷt danh sá ch đaቹu sá ch (ISBN, TenSach, TacGia) và soቷ sá ch có theቻ
mượ n. 
*/

if OBJECT_ID('Q8') is not null
	drop function Q8
go
create function Q8 ()
	returns table
as
	return (select ds.isbn, ds.tensach, ds.tacgia, count(distinct cs.masach) as amount
			from DauSach ds, CuonSach cs
			where ds.isbn = cs.isbn and cs.tinhtrang  = N'có thể mượn'
			group by ds.isbn, ds.tensach, ds.tacgia)
go
